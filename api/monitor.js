...
async function tavilySearch(query) {
  const res = await fetch("https://api.tavily.com/search", {
    ...
    body: JSON.stringify({ api_key: TAVILY_API_KEY, query, search_depth: "basic", max_results: 5, include_published_date: true }),
  });
...
async function scoreWithClaude(results) {
  const context = results.map((r, i) => `${i+1}. ${r.title} — ${r.content?.slice(0, 150) || ""}`).join("\n");
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-api-key": ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01" },
    body: JSON.stringify({ model: MODEL, max_tokens: 400, messages: [{ role: "user", content: `Score these IBM news results 0-10. 8-10=major launch/acquisition/earnings. 5-7=significant news. 1-4=regular content.\n\n${context}\n\nReturn ONLY JSON:\n{"importanceScore":0,"hasBreakingNews":false,"reason":"...","articles":[{"title":"...","url":"...","summary":"...","date":"..."}]}` }] }),
  });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`Monitor: got ${results.length} results`);
    ...
    if (!results.length) {
      ...
      await auditLog(ACTIONS.MONITOR_RUN, { importanceScore: 0, breakingNews: false, resultCount: 0 });
    ...
    if (result.breakingNews) await kvSet("monitor:alert", { ...result, dismissed: false }, 86400);

    // Audit log the monitor run
    await auditLog(ACTIONS.MONITOR_RUN, {
